...
public class CalcException {
    ...
    public static void action(int fOp, int sOp, String action) throws ZeroException {
        switch (action) {
            case "+": {
                org.usteniuk.Operation operation = org.usteniuk.Operation.SUM;
                operation.action(fOp, sOp, action);
                break;
            }
            case "-": {
                org.usteniuk.Operation operation = org.usteniuk.Operation.MINUS;
                operation.action(fOp, sOp, action);
                break;
            }
            case "*": {
                org.usteniuk.Operation operation = org.usteniuk.Operation.MULT;
                operation.action(fOp, sOp, action);
                break;
            }
            case "/": {
                org.usteniuk.Operation operation = org.usteniuk.Operation.DIV;
                operation.action(fOp, sOp, action);
                break;
            }
            case "%": {
                org.usteniuk.Operation operation = org.usteniuk.Operation.REM;
                operation.action(fOp, sOp, action);
                break;
            }
        }
